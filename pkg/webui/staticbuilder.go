...
func (swb *StaticWebuiBuilder) Build(path string) error {
	...
	for _, rs := range summaries {
		...
		gh.RunE(func() error {
			...
			err = os.WriteFile(filepath.Join(tmpDir, fmt.Sprintf("result-%s.js", rs.Id)), []byte(js), 0o600)
			if err != nil {
				return err
			}
			return nil
	...
	err = os.WriteFile(filepath.Join(tmpDir, "summaries.js"), []byte(j), 0o600)
	if err != nil {
		return err
	}
	...
	err = os.WriteFile(filepath.Join(tmpDir, "projects.js"), []byte(j), 0o600)
	if err != nil {
		return err
	}
	...
	err = os.WriteFile(filepath.Join(tmpDir, "shortnames.js"), []byte(j), 0o600)
	if err != nil {
		return err
	}

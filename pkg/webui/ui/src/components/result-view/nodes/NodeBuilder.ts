...
export class NodeBuilder {
  ...
  buildRoot(): [CommandResultNodeData, Map<string, NodeData>] {
    ...
    this.buildDeploymentProjectChildren(
      rootNode,
      this.props.commandResult.deployment!
    );

    if (this.deletedObjectsMap.size) {
      this.buildDeletedOrOrphanNode(
        rootNode,
        true,
        Array.from(this.deletedObjectsMap.values())
      );
    }
    if (this.orphanObjectsMap.size) {
      this.buildDeletedOrOrphanNode(
        rootNode,
        false,
        Array.from(this.orphanObjectsMap.values())
      );
    }
  ...
  buildDeploymentProjectChildren(
    node: NodeData,
  ...
  buildDeploymentItemNode(
    ...
    if (deploymentItem.path) {
      ...
      node = new DeploymentItemIncludeNodeData(
        this.props,
        newId,
        deploymentItem,
        deploymentItem.renderedInclude!
      );
      this.buildVarsSourceCollectionNode(node, deploymentItem.vars);
  ...
  buildDeletedOrOrphanNode(
    parentNode: NodeData,
    deleted: boolean,
    refs: ObjectRef[]
  ): DeletedOrOrphanObjectsCollectionNode {
    const idType = deleted ? "deleted" : "orphaned";
    const newId = `${parentNode.id}/(${idType})`;
    const node = new DeletedOrOrphanObjectsCollectionNode(
      this.props,
      newId,
      deleted
    );

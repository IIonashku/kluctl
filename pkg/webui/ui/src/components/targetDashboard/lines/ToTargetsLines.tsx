...
const FirstLineTop = styled("div")`
  ...
  left: 9px;
`;
const NextLineTop = styled("div", {
  shouldForwardProp: (prop) => prop !== "topX",
})<{ topX: number }>`
  position: absolute;
  ...
  height: ${({ topX }) =>
    `${topX > 0 ? (topX === 1 ? "160px" : "180px") : "60px"}`};
  width: calc(50% - 7px);
  bottom: ${({ topX }) =>
    `calc(${topX + 1} * ${
      topX > 0 ? (topX === 1 ? "30px" : "62px") : "20px"
    })`};
  left: 50%;
`;
...
const NextLineBottom = styled("div")`
  ...
  left: 50%;
`;
interface LineCenterProps {
  width?: number;
  circle?: boolean;
}
export const LineCenter = styled("div", {
  shouldForwardProp: (prop) => prop !== "width" && prop !== "circle",
})<LineCenterProps>`
  position: absolute;
  top: 50%;
  left: ${({ width }) => (width ? `-${width + 12}px` : "50%")};
  transform: ${({ width }) =>
    width ? "translate( 0, -50%)" : "translate(-50%, -50%)"};
  width: ${({ width }) => (width ? width + "px" : "calc(100% - 9px * 2)")};
  height: 2px;
  background: #000;
  ::after {
    content: "";

...
class MyProvider implements SidePanelProvider {
  ...
  buildSidePanelTabs(): SidePanelTab[] {
    ...
    const tabs = [{ label: "Summary", content: this.buildSummaryTab() }];
  ...
  buildSummaryTab(): React.ReactNode {
    ...
    if (this.ts?.lastValidateResult) {
      props.push({
        name: "Ready",
        value: this.ts.lastValidateResult.ready + "",
      });
    }
    if (this.ts?.lastValidateResult?.errors?.length) {
      props.push({
        name: "Errors",
        value: this.ts.lastValidateResult.errors.length + "",
      });
    }
    if (this.ts?.lastValidateResult?.warnings?.length) {
      props.push({
        name: "Warnings",
        value: this.ts.lastValidateResult.warnings.length + "",
      });
    }
    if (this.ts?.lastValidateResult?.drift?.length) {
      props.push({
        name: "Drifted Objects",
        value: this.ts.lastValidateResult.drift.length + "",
      });
    }
...
export const TargetDetailsDrawer = (props: {
  ts?: TargetSummary;
  onClose: () => void;
}) => {
  return (

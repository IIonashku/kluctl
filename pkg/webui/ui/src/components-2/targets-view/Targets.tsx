...
const StatusIcon = (props: { ps: ProjectSummary; ts: TargetSummary }) => {
  ...
  if (props.ts.lastValidateResult === undefined) {
    ...
    props.ts.lastValidateResult.ready &&
    !props.ts.lastValidateResult.errors
  ) {
  ...
  if (props.ts.lastValidateResult === undefined) {
    ...
    if (
      props.ts.lastValidateResult.ready &&
      !props.ts.lastValidateResult.errors?.length
    ) {
    ...
    if (props.ts.lastValidateResult.errors?.length) {
      tooltip.push(
        `Target has ${props.ts.lastValidateResult.errors.length} validation errors.`
      );
    }
    if (props.ts.lastValidateResult.warnings?.length) {
      tooltip.push(
        `Target has ${props.ts.lastValidateResult.warnings.length} validation warnings.`
      );
    }
    if (props.ts.lastValidateResult.drift?.length) {
      tooltip.push(
        `Target has ${props.ts.lastValidateResult.drift.length} drifted objects.`
      );
    }
  }
...
export const TargetItem = (props: {
  ...
  props.ts.commandResults?.forEach((rs) => {
    ...
    handleContext(rs.commandInfo.target?.context);
  });
        ...
        <Box>
          <Box display={"flex"} alignItems={"center"}>
            <Box display={"flex"} width={"100%"}>
              ...
              <Tooltip
                title={"Discriminator: " + props.ts.target.discriminator}
              >
                <div>
                  <Fingerprint />
                </div>
